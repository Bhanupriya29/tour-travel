...
function Dashboard(props) {
    ...
    const covidapi = "https://data.covid19india.org/v4/data.json";
    const api_url = 'https://data.covid19india.org/states_daily.json';
    let elt;
    let active;
    ...
    async function buildTable() {
        const response = await fetch('https://data.covid19india.org/v4/data.json');
        const response2 = await fetch("https://corona.lmao.ninja/v2/countries");
        var data2 = await response2.json();
        data2.forEach(elt => {
            if (elt.country === 'India') { data2 = elt; return; }
